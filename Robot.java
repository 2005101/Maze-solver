...
import javax.swing.JLabel;// awt is the graphic package 
import javax.swing.JOptionPane;// graphic stuff
...
public class Robot {
	...
	int x; // this is the coordinate system thing 
	int y;
    public int length;
	...
	public Robot(DoorRoom room, int sx, int sy) {
		this.room = room; // this for the location, check LATER??
		this.setX(sx);
		this.setY(sy);
		this.show();
	 ...
	 * @param grid grid[x][y] = true if there is a room in that spot, false otherwise
	 */
	public Robot(boolean[][] grid) {// This is constructors
	 ...
	 */
	public boolean check(char dir) { // It checks for a door!
		try { Thread.sleep(125); } catch (InterruptedException e) {}
		return room.isDoor(this.getX(), this.getY(), dir); 
	}
	/**
	 * Look for a door
	 * @param dir one of "n", "s", "e", or "w". Capitalization does not matter.
	 ...
	 * @param dir one of 'n', 's', 'e', or 'w'. Capitalization does not matter.
	 */
	public void go(char dir) {// it moves the robot across the room 
		if (!this.check(dir)) {
			switch(dir) {
		...
		switch(dir) {
		case 'w': case 'W': for(double i=0; i<1; i+=0.03125) this.show(-i,0); this.setX(this.getX() - 1); break;
		case 'e': case 'E': for(double i=0; i<1; i+=0.03125) this.show(i,0); this.setX(this.getX() + 1); break;
		case 'n': case 'N': for(double i=0; i<1; i+=0.03125) this.show(0,-i); this.setY(this.getY() - 1); break;
		case 's': case 'S': for(double i=0; i<1; i+=0.03125) this.show(0,i); this.setY(this.getY() + 1); break;
		default: throw new IllegalArgumentException("Unknown direction: "+dir);
		}
		this.show();
	}
	...
	private  void show() {
		this.room.drawBot(this.getX(), this.getY());
	}
	private void show(double dx, double dy) {
		this.room.drawBot(this.getX()+dx, this.getY()+dy);
	 ...
	 */
	public void say(Object msg) {// it can say something 
